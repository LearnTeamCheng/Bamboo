...
>
> ### ⚠️ 先读这一条
> 本文档列出的 37 项缺陷是**症状**，不是根因。它们来自**五个架构缺陷**：
> 缺少「执行顺序」「对象生命周期」「资源身份」「数据模式」「渲染契约」的**唯一权威**。
> **根因分析、目标架构与迁移路径见 [`docs/architecture_upgrade.md`](architecture_upgrade.md)。**
> 只按本文档逐条修 bug，架构不变，新代码会继续长出同样的形状。

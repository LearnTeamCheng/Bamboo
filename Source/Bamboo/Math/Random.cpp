...
namespace Bamboo
{
    ...
    template <>
    int Random::Range<int>(int min, int max) noexcept { return Int(min, max); }
    template <>
    float Random::Range<float>(float min, float max) noexcept { return Float(min, max); }
    template <>
    double Random::Range<double>(double min, double max) noexcept { return Double(min, max); }
